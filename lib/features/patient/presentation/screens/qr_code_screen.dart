...
class _QrCodeScreenState extends ConsumerState<QrCodeScreen>
          ...
          children: [
                  ...
                  return GestureDetector(
                    ...
                    child: MouseRegion(
                      onHover: (event) => _updateTilt(
                          event.localPosition, cardWidth, cardHeight),
                      onExit: (_) => _resetTilt(),
                      child: Listener(
                        onPointerDown: (event) => _updateTilt(
                            event.localPosition, cardWidth, cardHeight),
                        onPointerMove: (event) => _updateTilt(
                            event.localPosition, cardWidth, cardHeight),
            ...
            Center(
              child: Text(
                ...
                style: TextStyle(fontSize: 11, color: t.textSecondary, height: 1.6),
              ),
            ),
          ],
        ),
  ...
  Widget _buildConditionRow(MedicalCondition cond) {
    final t = context.tokens;
    final severityColor = _severityColor(cond.severity);
    return SquircleCard(
      radius: AppSpacing.squircleGrouped,
      borderSide: BorderSide(color: t.divider),
