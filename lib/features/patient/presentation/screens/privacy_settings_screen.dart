...
class PrivacySettingsScreen extends ConsumerWidget {
            ...
            child: Column(
              ...
              children: [
                ...
                SquircleCard(
                  ...
                  color: t.tint,
                  borderSide: BorderSide(color: t.accent.withValues(alpha: 0.2)),
                  padding:
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildSettingRow(
                        ...
                        isPublic: true,
                        locked: true,
                        onEdit: () => _showBloodTypeSheet(
                            context, ref, patient?['blood_type']),
                      ),
                      Divider(height: 1, color: t.divider),
                      _buildSettingRow(
                        context,
                        ...
                        isPublic: true,
                        locked: true,
                        onEdit: () => _showEmergencyContactSheet(
                            context, ref, patient?['emergency_contact']),
                      ),
                    ],
                  ),
                ),
  ...
  Widget _buildSummaryCard(
      BuildContext context, String conditions, String prescriptions) {
    final t = context.tokens;
  ...
  Future<void> _showBloodTypeSheet(
      BuildContext context, WidgetRef ref, String? currentType) async {
  ...
  Future<void> _showEmergencyContactSheet(
      BuildContext context, WidgetRef ref, Map<String, dynamic>? current) async {
    final nameController = TextEditingController(text: current?['name']);
    final phoneController = TextEditingController(text: current?['phone']);
    final relationshipController =
        TextEditingController(text: current?['relationship']);
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                field('PHONE NUMBER', phoneController, 'Enter phone number',
                    keyboard: TextInputType.phone),
                field('RELATIONSHIP', relationshipController,
                    'e.g., Spouse, Parent'),
                ...
                CSTwoButtonRow(
                  ...
                  onConfirm: () async {
                    ...
                    await _updatePatientField(
                        context, ref, 'emergency_contact', contact);
                  },
                ),
              ],
            ),
  ...
  Future<void> _updatePatientField(
      BuildContext context, WidgetRef ref, String field, dynamic value) async {
    try {
      final supabase = Supabase.instance.client;
      final userId = supabase.auth.currentUser?.id;
  ...
  Future<void> _makeAllConditionsPrivate(
    ...
    try {
      ...
      await supabase
          .from('medical_conditions')
          .update({'is_public': false}).eq('patient_id', patientResult['id']);
  ...
  Future<void> _makeAllConditionsPublic(
    ...
    try {
      ...
      await supabase
          .from('medical_conditions')
          .update({'is_public': true}).eq('patient_id', patientResult['id']);
  ...
  Future<void> _regenerateQrCode(BuildContext context, WidgetRef ref) async {
    ...
    try {
      ...
      await supabase
          .from('patients')
          .update({'qr_code_id': newQrCodeId}).eq('user_id', userId);
